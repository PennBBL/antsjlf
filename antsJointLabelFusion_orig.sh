...
for (( i = 0; i < ${#ATLAS_IMAGES[@]}; i++ ))
  do
    ...
    regcall=${ANTSPATH}/antsRegistrationSyN_JLF.sh
    if [[ $RUNQUICK -eq 1 ]];
      then
        # Don't do this, bad registrations. Can save time without going to this extreme
        echo " QUICKER REGISTRATION NOT AVAILABLE " 
        exit 1
    ...
    if [[ $DOQSUB -eq 1 ]];
      then
        id=`qsub -cwd -S /bin/bash -N antsJlfReg -v ANTSPATH=$ANTSPATH -j y -o ${OUTPUT_PREFIX}${BASENAME}_${i}_qsubLog.txt -pe unihost 2 -binding linear:2 -l h_vmem=8G,s_vmem=8G $QSUB_OPTS $qscript | awk '{print $3}'`
        jobIDs="$jobIDs $id"
        sleep 0.5
    elif [[ $DOQSUB -eq 4 ]];
      then
...
if [[ $DOQSUB -eq 1 ]];
  then
    ...
    ${ANTSPATH}/waitForSGEQJobs.pl 0 3600 $jobIDs
    ...
    # Use multi-threading here because this is a bottleneck
    jobIDs=`qsub -cwd -S /bin/bash -N antsJlf -v ANTSPATH=$ANTSPATH -j y -o ${OUTPUT_PREFIX}antsJointFusionLog.txt -l h_vmem=12G,s_vmem=12G -pe unihost 2 -binding linear:2 $QSUB_OPTS $qscript2 | awk '{print $3}'`
    ${ANTSPATH}/waitForSGEQJobs.pl 0 1800 $jobIDs
  fi
if [[ $DOQSUB -eq 4 ]];
  then
