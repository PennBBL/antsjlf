...
for (( i = 0; i < ${#ATLAS_IMAGES[@]}; i++ ))
  do
    ...
    if [[ $DOQSUB -eq 1 ]];
      then
        id=`qsub -cwd -S /bin/bash -N antsJlfReg -v ANTSPATH=$ANTSPATH -j y -o ${OUTPUT_PREFIX}${BASENAME}_${i}_qsubLog.txt -l h_vmem=5.5G,s_vmem=5G $QSUB_OPTS $qscript | awk '{print $3}'`
        jobIDs="$jobIDs $id"
        sleep 0.5
    elif [[ $DOQSUB -eq 4 ]];
      then
...
if [[ $DOQSUB -eq 1 ]];
  then
    ...
    ${ANTSPATH}/waitForSGEQJobs.pl 1 900 $jobIDs
    ...
    jobIDs=`qsub -cwd -S /bin/bash -N antsJlf -v ANTSPATH=$ANTSPATH -j y -o ${OUTPUT_PREFIX}antsJointFusionLog.txt -l h_vmem=6.5G,s_vmem=6G -pe unihost 1 -binding linear:1 $qscript2 | awk '{print $3}'`
